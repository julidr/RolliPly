...
public class GameManager : MonoBehaviour
{
    ...
    public void addBall(GameObject ball, float minimumDistance)
    {
        Transform target = player.GetComponent<Transform>();
        Vector3 ballPosition = new Vector3(target.position.x+0.05f, target.position.y, target.position.z);

        GameObject newBall = Instantiate(ball, ballPosition, Quaternion.identity);
        Color randomColor = new Color(Random.Range(0f, 1f), Random.Range(0f, 1f), Random.Range(0f, 1f));
        newBall.transform.LookAt(target);
        newBall.GetComponent<MeshRenderer>().material.SetColor("_Color", randomColor);
